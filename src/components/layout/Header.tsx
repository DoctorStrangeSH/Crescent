...
function Header() {
  ...
  const handleClick = (gameId: string, parentId: string | null) => {
    ...
    navigate(parentId ? `/game/${parentId}` : `/game/${gameId}`);
  };
  ...
  return (
    <header className="flex items-center justify-between h-14 px-4 lg:px-6 bg-white dark:bg-surface-dark border-b border-surface-border dark:border-surface-border-dark">
      ...
      <div ref={searchRef} className="hidden sm:block flex-1 max-w-sm lg:ml-0 ml-4 relative">
        <div className="relative">
          ...
          <input type="text" value={searchQuery} onChange={e => { setSearchQuery(e.target.value); setShowResults(true); }} onFocus={() => setShowResults(true)}
            placeholder="Поиск..." className="block w-full pl-9 pr-9 py-1.5 bg-surface-hover dark:bg-surface-hover-dark border border-surface-border dark:border-surface-border-dark rounded-xl text-sm text-gray-900 dark:text-white placeholder-surface-muted focus:outline-none focus:ring-2 focus:ring-crescent-accent/50 transition-all" />
        ...
        {showResults && searchQuery.trim() && (
          <div className="absolute top-full mt-1 left-0 right-0 bg-white dark:bg-surface-card-dark rounded-xl shadow-xl border border-surface-border dark:border-surface-border-dark py-1 z-50 max-h-80 overflow-y-auto">
            {filtered.length > 0 ? filtered.map(g => (
              <button key={g.id} onClick={() => handleClick(g.id, g.parentId)} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-left hover:bg-surface-hover dark:hover:bg-surface-hover-dark transition-colors">
                <span className="text-base">{g.status === 'owned' ? '✅' : '🎯'}</span>
                <div className="min-w-0"><p className="font-medium text-gray-900 dark:text-white truncate">{g.title}</p></div>
                {g.parentId && <span className="text-[10px] text-surface-muted ml-auto">📦 доп</span>}
              </button>
      ...
      <div className="flex items-center gap-1">
        <ExportMenu />
        <BackupMenu />
        <button onClick={toggleTheme} className="p-2 rounded-xl text-surface-muted hover:text-gray-700 dark:hover:text-white hover:bg-surface-hover dark:hover:bg-surface-hover-dark transition-colors">
