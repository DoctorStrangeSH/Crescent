    ...
    <div className="p-6 lg:p-8 space-y-8 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-surface-muted mt-0.5">Обзор коллекции</p>
        </div>
      ...
      <div className="bg-white dark:bg-surface-card-dark rounded-2xl border border-surface-border dark:border-surface-border-dark shadow-card dark:shadow-card-dark p-5">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-semibold text-gray-900 dark:text-white flex items-center gap-2"><TrendingUp className="w-4 h-4 text-crescent-accent" />Общий прогресс</h3>
          <span className="text-lg font-bold text-crescent-accent">{totalProgress}%</span>
        </div>
        <div className="h-3 bg-surface-hover dark:bg-surface-hover-dark rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-crescent-accent to-crescent-gold rounded-full transition-all duration-1000" style={{ width: `${totalProgress}%` }} />
        </div>
      ...
      {statusData.length > 0 && (
        <div className="bg-white dark:bg-surface-card-dark rounded-2xl border border-surface-border dark:border-surface-border-dark shadow-card dark:shadow-card-dark p-5">
          ...
          <div className="flex justify-center gap-6 mt-2">
            {statusData.map(d => <div key={d.name} className="flex items-center gap-2 text-sm"><div className="w-3 h-3 rounded-full" style={{ backgroundColor: d.color }} /><span className="text-surface-muted">{d.name}</span><span className="font-semibold text-gray-900 dark:text-white">{d.value}</span></div>)}
          </div>
        </div>
      )}
    </div>
...
function StatCard({ label, value, subtitle, icon, color = 'default' }: { label: string; value: string | number; subtitle?: string; icon: React.ReactNode; color?: string }) {
  const bg: Record<string, string> = {
    default: 'bg-surface-hover dark:bg-surface-hover-dark',
    green: 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400',
    gold: 'bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400',
    ...
    <div className="bg-white dark:bg-surface-card-dark rounded-2xl border border-surface-border dark:border-surface-border-dark shadow-card dark:shadow-card-dark p-4">
      <div className="flex items-start justify-between">
        <div>
          ...
          {subtitle && <p className="text-[11px] text-surface-muted mt-0.5">{subtitle}</p>}
        </div>
        <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${bg[color] || bg.default}`}>{icon}</div>
      </div>
    </div>
  );
}
