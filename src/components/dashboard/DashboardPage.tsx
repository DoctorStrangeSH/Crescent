...
function DashboardPage() {
  ...
  const statusData = [
    ...
    { name: 'Хочу', value: stats.wishlistGames, color: '#f0a860' },
  ].filter(d => d.value > 0);
  ...
  if (stats.totalGames === 0) {
    return (
      <div className="flex items-center justify-center min-h-[80vh]">
        <EmptyState icon={<Library className="w-12 h-12 text-white/30" />} title="Добро пожаловать в Crescent" description="Добавьте свою первую игру" actionLabel="Перейти в коллекцию" onAction={() => navigate('/collection')} />
      </div>
    );
  }
    ...
    <div className="p-6 lg:p-8 space-y-8 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-white/40 mt-0.5">Обзор коллекции</p>
        </div>
      ...
      {statusData.length > 0 && (
        <div className="bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10 p-5">
          ...
          <div className="flex justify-center gap-6 mt-2">
            {statusData.map(d => <div key={d.name} className="flex items-center gap-2 text-sm"><div className="w-3 h-3 rounded-full" style={{ backgroundColor: d.color }} /><span className="text-white/60">{d.name}</span><span className="font-semibold text-white">{d.value}</span></div>)}
          </div>
        </div>
      )}
    </div>
