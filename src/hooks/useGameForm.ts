...
const schema = z.object({
  ...
  kind: z.enum(['base', 'expansion', 'standalone']).optional().default('standalone'),
...
export function formDataToCreateData(data: GameFormData): CreateGameData {
  return {
    ...
    kind: (data.kind || 'standalone') as 'base' | 'expansion' | 'standalone',
    title: data.title, titleOriginal: data.titleOriginal || '', year: data.year ?? null,
    publisher: data.publisher || '', designers: strToArr(data.designers || ''), artists: strToArr(data.artists || ''),
    ...
    age: null, complexity: (data.complexity || 2) as 1|2|3|4|5,
    bggRating: null, bggLink: '', teseraLink: '', hobbygameLink: '',
    genres: strToArr(data.genres || ''), mechanics: strToArr(data.mechanics || ''),
    ...
    tags: strToArr(data.tags || ''), missingComponents: '', acquisitionSource: '',
    sortOrder: 0,
  };
}

export function useGameForm(defaultValues?: Partial<GameFormData>) {
  const collectionId = useUIStore(s => s.collectionId);
  return useForm<GameFormData>({
    resolver: zodResolver(schema),
    defaultValues: {
      collectionId: collectionId || null, kind: 'standalone',
